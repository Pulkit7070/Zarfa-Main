...
export function FundTransferStep({
  ...
  const handleTransferToken = async (index: number) => {
    ...
    try {
      ...
      const availableBalance = token.isNative
        ? (nativeBalance?.value || 0n)
        : (token.eoaBalance || 0n);

      if (amountWei > availableBalance) {
        const balanceFormatted = token.isNative
          ? formatEther(availableBalance)
          : formatUnits(availableBalance, token.decimals);
        const amountFormatted = token.isNative
          ? formatEther(amountWei)
          : formatUnits(amountWei, token.decimals);
        throw new Error(
          `Insufficient balance. You have ${balanceFormatted} ${token.symbol}, but trying to transfer ${amountFormatted} ${token.symbol}`
        );
      }
