...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const [backendToken, setBackendToken] = useState<string | null>(() => {
    try {
      return localStorage.getItem('agmon_backend_token');
    } catch {
      return null;
    }
  });
  const [tokenExpiry, setTokenExpiry] = useState<number>(() => {
    try {
      const expiry = localStorage.getItem('agmon_token_expiry');
  ...
  const getBackendToken = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      try {
        localStorage.setItem('agmon_backend_token', authResponse.accessToken);
        localStorage.setItem('agmon_token_expiry', authResponse.expiresAt.toString());
      } catch (error) {
  ...
  const clearBackendAuth = useCallback(() => {
    ...
    try {
      localStorage.removeItem('agmon_backend_token');
      localStorage.removeItem('agmon_token_expiry');
    } catch (error) {
