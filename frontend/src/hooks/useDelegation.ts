...
export function useDelegation(chainId?: number) {
  ...
  const createDelegation = useCallback(
    async (
      ...
      // CRITICAL VALIDATION: If strategyId is provided, verify signing wallet matches strategy owner
      if (strategyId) {
        try {
          const token = await getBackendToken();
          if (token) {
            const { strategiesApi } = await import('@/lib/api/strategies');
            const strategy = await strategiesApi.getStrategy(strategyId, token);
            
            // Check if signing wallet matches strategy owner
            if (strategy.userAddress.toLowerCase() !== userAddress.toLowerCase()) {
              const errorMsg = `Wallet mismatch! Strategy is owned by ${strategy.userAddress}, but you're signing with ${userAddress}. Please switch to the correct wallet that owns this strategy.`;
              toast({
                title: 'Wallet Mismatch',
                description: errorMsg,
                variant: 'destructive',
              });
              console.error('❌ Strategy ownership mismatch:', {
                strategyOwner: strategy.userAddress,
                signingWallet: userAddress,
                strategyId,
              });
              return null;
            }
          ...
          // If we can't fetch strategy, log warning but continue (may be race condition)
          console.warn('Could not verify strategy ownership before delegation:', error.message);
        }
      }
      ...
      try {
        ...
        // CRITICAL: Verify wallet is using the correct account before signing
        // Phantom/MetaMask may use a different account if user switched accounts
        try {
          // Get wallet type for better error messages
          const walletType = privyWallet.walletClientType || 'wallet';
          const walletName = walletType === 'privy' ? 'Embedded Wallet' : 
                            privyWallet.connectorType === 'phantom' ? 'Phantom' :
                            privyWallet.connectorType === 'metamask' ? 'MetaMask' : 'Wallet';
          
          // Try multiple methods to get active account (Phantom may expose differently)
          let activeAccount: string | null = null;
          
          // Method 1: eth_accounts
          try {
            const accounts = await provider.request({ method: 'eth_accounts' });
            activeAccount = accounts?.[0]?.toLowerCase() || null;
          } catch (e) {
            console.warn('eth_accounts failed, trying alternative method:', e);
          }
          
          // Method 2: If eth_accounts fails, try privyWallet.address directly
          if (!activeAccount) {
            activeAccount = privyWallet.address?.toLowerCase() || null;
          }
          
          const expectedAccount = userAddress.toLowerCase();
          
          if (activeAccount && activeAccount !== expectedAccount) {
            const errorMsg = `Account mismatch! ${walletName} is using account ${activeAccount}, but expected ${expectedAccount}. Please switch to the correct account in ${walletName}.`;
            ...
            toast({
              ...
              description: `Please switch to account ${expectedAccount.slice(0, 6)}...${expectedAccount.slice(-4)} in ${walletName}`,
              variant: 'destructive',
            });
          ...
          if (!activeAccount) {
            console.warn('⚠️ Could not determine active account, proceeding with signTypedData...');
          } else {
            console.log('✅ Account verified:', { 
              wallet: walletName,
              expected: expectedAccount, 
              actual: activeAccount 
            });
          }
        } catch (accountError: any) {
          // If account check fails, still try signing (may be provider issue)
          console.warn('⚠️ Could not verify account:', accountError.message);
          if (accountError.message.includes('Account mismatch')) {
            throw accountError; // Re-throw account mismatch errors
          }
        }

        // DEBUG: Log wallet details before signing (especially important for Phantom)
        console.log('🔍 DEBUG - About to sign delegation:');
        console.log('  - Wallet type:', privyWallet.walletClientType || 'unknown');
        console.log('  - Connector type:', privyWallet.connectorType || 'unknown');
        ...
        // Phantom-specific: Log provider info
        if (privyWallet.connectorType === 'phantom' || provider?.isPhantom) {
          console.log('  - ⚠️ Phantom wallet detected - account verification is critical');
        }
