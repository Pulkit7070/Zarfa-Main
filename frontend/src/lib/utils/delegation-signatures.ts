...
export async function signDelegation(
  ...
  try {
    ...
    if (provider?.request) {
      ...
      // Try to get active account (works with MetaMask, Phantom, etc.)
      try {
        const accounts = await provider.request({ method: 'eth_accounts' });
        activeAccount = accounts?.[0]?.toLowerCase() || null;
      } catch (e) {
        // Some wallets (like Phantom) may not support eth_accounts
        // or may expose accounts differently
        console.warn('Could not get accounts via eth_accounts:', e);
      }
      
      // Fallback: Use the account from walletClient if available
      if (!activeAccount && walletClient.account) {
        activeAccount = walletClient.account.address.toLowerCase();
      }
      ...
      if (activeAccount && activeAccount !== expectedAccount) {
        console.error('❌ Account mismatch detected!', {
          ...
          walletClientAccount: walletClient.account?.address?.toLowerCase(),
          message: 'Wallet is using a different account than expected.',
        });
        throw new Error(
          `Account mismatch: Expected ${expectedAccount}, but wallet is using ${activeAccount}. ` +
          `Please switch to account ${expectedAccount.slice(0, 6)}...${expectedAccount.slice(-4)} in your wallet and try again.`
        );
      }
      ...
      if (activeAccount) {
        ...
        console.warn('⚠️ Could not verify account before signing - proceeding anyway');
      }
    }
